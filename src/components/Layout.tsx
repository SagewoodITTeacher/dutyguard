...
import { LogOut, Bell, Shield, User, Menu, X, LayoutDashboard, Calendar, Users, MapPin } from 'lucide-react';
...
export function Layout({ session }: { session: any }) {
  ...
  const navItems = [
    { path: '/teacher', label: 'Timeline', icon: Calendar },
    { path: '/manager', label: 'Analytics', icon: LayoutDashboard },
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-4">
              <div className="flex flex-col items-end">
                <span className="text-sm font-black tracking-tight text-slate-900 italic">{userEmail?.split('@')[0]}</span>
                <div className="flex items-center gap-1.5 opacity-40">
                  <span className="h-1 w-1 rounded-full bg-emerald-500"></span>
                  <span className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-500">Active Duty</span>
                </div>
              </div>
              <button 
